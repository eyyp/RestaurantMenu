    ...
    return( 
        <View style={styles.button}>
            {imageUri ? 
                ...
                />
                : <View style={{backgroundColor:'#ffdec4',width:horizontalScale(75),height:verticalScale(74),borderRadius:15}}>
                    <ActivityIndicator 
            ...
            <View style={{flexDirection:'row',justifyContent:'space-between',width:horizontalScale(240),paddingTop:verticalScale(5)}}>
                <View style={{marginLeft:horizontalScale(10)}}>
                    <Text style={{fontSize:24,color:'#F4E6CD'}}>{data?.title}</Text>
                    {propertys.map((item,index)=>
                        <PropertyText key={index} item={item}/>
                    )}
                    <Text style={{fontSize:20,fontWeight:'500',color:'#FE9870'}}>{data?.price}₺</Text>
                </View>
            </View>
        </View>
    )
