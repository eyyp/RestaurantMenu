...
import { TypeCreate } from "../../services/methods/property";
...
const PropertyTypeAdd = (props)=>{
    ...
    const CreateRequest = ()=>{
        if(propertyType === "" || propertyType === null || propertyType === undefined){
            setErrorPropertyType("Lütfen bir isim giriniz");
        }
        else{
            TypeCreate(userCheck.restaurant_id,"1",propertyType,check,(err,data)=>{
                if(data){
    ...
    return(
        <View style={styles.content}>
            ...
            <Text>Özellik Grubu Ekle</Text>
            <TextField
                style={styles.textField} 
                value={propertyType}
                label="Özellik Grubu Başlığı"
                errorText={errorPropertyType}
                onChangeText={(text) => setPropertyType(text)}
            />
            <TouchableOpacity style={styles.checkButton} onPress={()=>SetCheckhBox()}>
                {check === "true"
                    ?<Image 
                        style={styles.check}
                ...
                <Text style={styles.checkText}>Zorunlu olsun mu</Text>
            </TouchableOpacity>
